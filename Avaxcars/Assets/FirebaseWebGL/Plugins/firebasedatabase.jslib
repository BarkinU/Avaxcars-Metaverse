mergeInto(LibraryManager.library, {
    ...
    GetJSON: function(path, objectName, callback, fallback) {
        ...
        try {
            ...
            firebase.database().ref(parsedPath).once('value').then(function(snapshot) {
                ...
                unityInstance.Module.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(childData));
                });
            });
    ...
    PushJSON: function(path, value, objectName, callback, fallback) {
        var parsedPath = Pointer_stringify(path);
        var parsedValue = Pointer_stringify(value);
        var obj = JSON.parse(parsedValue);
        ...
        try {
            ...
            firebase.database().ref(parsedPath).push().set(obj).then(function(unused) {
    ...
    ListenForChildAdded: function(path, objectName, callback, fallback) {
        ...
        try {
            ...
            firebase.database().ref(parsedPath).orderByKey().limitToLast(100).on('child_added', function(snapshot) {
                unityInstance.Module.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(snapshot.val()));
            });
